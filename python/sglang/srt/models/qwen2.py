...
class Qwen2Attention(nn.Module):
    ...
    def _forward_with_fp8_input(
        ...
        reusing FP8 already computed by the preceding fused RMSNorm+FP8 op.
        x is the float tensor before norm, used only for output dtype/shape.
        skip_o_reduce: if True, skip the o_proj allreduce so the caller can fuse
        allreduce + post_attention_layernorm via forward_with_allreduce_fusion_fp8_out.
        """
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(
        ...
        # Currently only validated for TP=1; TP>1 has allreduce fusion issues.
    ...
    def _forward_aiter_fp8(
        ...
        For TP>1, also fuses the o_proj allreduce with post_attention_layernorm
        via forward_with_allreduce_fusion_fp8_out when AITER custom-AR is available.
        """
