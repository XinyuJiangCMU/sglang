...
class MiMoV2DecoderLayer(nn.Module):
    ...
    def _forward_aiter_fp8(
        ...
        """MiMoV2 decoder forward with fused add+RMSNorm+FP8 quantization (AMD AITER).

        Uses LayerCommunicator.prepare_attn_fp8_out() to fuse the input_layernorm
        add+norm+per_token_quant into a single kernel call.
        For dense (non-MoE) layers, also fuses post_attention_layernorm via
        prepare_mlp_fp8_out() when should_allreduce_fusion is False.
        ...
        # For MoE layers or when allreduce/scatter fusion is requested, fall back to
        # standard MLP path (the FP8 MLP path always reduces in down_proj).
        if self.is_layer_sparse or should_allreduce_fusion or use_reduce_scatter:
            return self._forward_mlp_standard(hidden_states, residual, forward_batch)

        # Dense layer: fused post_attention_layernorm+FP8 quant
