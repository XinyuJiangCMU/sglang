...
class MixtralDecoderLayer(nn.Module):
    ...
    def _forward_aiter_fp8(
        ...
        """MixtralQuant decoder forward with fused add+RMSNorm+FP8 for attention (AMD AITER).

        Fuses the input_layernorm add+norm+per_token_quant into a single kernel
        (~13µs savings per layer on MI300X). The post_attention_layernorm uses
        the standard allreduce+norm path since MoE does not accept FP8 input.
        """
        ...
        # MoE MLP: allreduce (TP>1) + standard RMSNorm (MoE doesn't support FP8 input)
