...
class Qwen3Attention(nn.Module):
    ...
    def _forward_with_fp8_input(
        ...
        Note: Qwen3 has per-head q_norm/k_norm so QKV output is BF16 from here.
        """
...
class Qwen3DecoderLayer(nn.Module):
    ...
    def _forward_aiter_fp8(
        ...
        """Qwen3 decoder forward with fused add+RMSNorm+FP8 quantization (AMD AITER).
        ...
        Falls back to the standard path via prepare_attn/prepare_mlp when the
        fused FP8 path is unavailable (complex TP/DP scatter modes, allreduce
        fusion not available for TP>1, etc.).
        """
        ...
        # MLP: fused allreduce+add+norm+fp8 quant.
        # prepare_mlp_fp8_out handles allreduce for TP>1 (AITER AR fusion) or
        # plain add+norm+fp8 for TP=1.  Returns None when the fused FP8 path is
        # unavailable.  In that case, fall back to prepare_mlp which handles
        # allreduce + layernorm internally (for both TP=1 and TP>1).
        ...
        if mlp_fp8 is not None:
            ...
            # Fallback: prepare_mlp handles allreduce (TP>1) + layernorm
            return self._forward_mlp_standard(hidden_states, residual, forward_batch)
